package xstampp.stlsa.ui.results;
...
public class ResultEditor extends StandartEditorPart implements Observer {
  ...
  public TableViewerColumn getIdColumn() {
    return this.stlsastepColumn;
  }

  public void setIdColumn(TableViewerColumn idColumn) {
    this.stlsastepColumn = idColumn;
  }
  ...
  public void createPartControl(Composite parent) {
              ...
              try {
                ...
                if (view == null) {
                  view = (DiagramView) PlatformUI.getWorkbench().getActiveWorkbenchWindow()
                      .getActivePage().showView("xstampp.stlsa.diagram");
    ...
    CellEditor[] controlActionEditors = new CellEditor[6];
    ...
    updatelabel();
    this.stlsastepColumn = new TableViewerColumn(this.getTableViewer(), SWT.NONE);
    this.stlsastepColumn.getColumn().setText(STPA_SEC_STEP);
    final int stlsaWeight = 20;
    final int stlsaMinWidth = 39;
    this.tableColumnLayout.setColumnData(this.stlsastepColumn.getColumn(),
        new ColumnWeightData(stlsaWeight, stlsaMinWidth, true));
    ...
    tableViewer.getTable().addListener(SWT.PaintItem, new Listener() {
      ...
      public void handleEvent(Event event) {
        ...
        if ((event.index == 5) & (event.item.getData().getClass() == ConstraintResult.class)) {
        ...
        // else if((event.index == 2) & (event.item.getData().getClass()
        // == ConstraintResult.class)){
        // ConstraintResult entry = (ConstraintResult)
        // event.item.getData();
        // DiagramView view = new DiagramView(entry.getTemp());
  ...
  private void updatelabel() {
    filterLabel2.setText("Security related: " + this.dataInterface.getConstraintController().getSecureValueCount()
        + "  Safety related: " + this.dataInterface.getConstraintController().getSafeValueCount() + "  Both: "
