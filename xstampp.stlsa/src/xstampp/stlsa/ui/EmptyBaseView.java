 ...
 * 
 */
public abstract class EmptyBaseView<T extends IDataModel> extends StandartEditorPart {
...
//  }
   ...
   */
  public EmptyBaseView(boolean restrict) {
  ...
  public EmptyBaseView(EnumSet<TableStyle> style) {
    this();
    if (style != null) {
      this.style = style;
    }
  ...
  public EmptyBaseView(EnumSet<TableStyle> style, String tableHeader) {
    this(style);
    this.tableHeader = tableHeader;
  }

  public EmptyBaseView(String tableHeader) {
    this();
    this.tableHeader = tableHeader;
  }
...
//    }
  ...
  private void createTableViewer(Composite tableComposite) {
    ...
    this.getTableViewer().getTable().addKeyListener(new KeyListener() {
      ...
      public void keyPressed(KeyEvent e) {
        if (((e.stateMask == SWT.CTRL) || (e.stateMask == SWT.COMMAND)) && (e.keyCode == 'a')
            && (EmptyBaseView.this.getTableViewer().getTable().getItemCount() > 0)) {
          EmptyBaseView.this.tableViewer.setSelection(
              new StructuredSelection(EmptyBaseView.this.getTableViewer().getElementAt(0)), true);
          EmptyBaseView.this.tableViewer.getTable().selectAll();
        }
      }
  ...
  private void createButtonBar() {
    ...
    deleteItemsButton.addListener(SWT.Selection, new Listener() {
      ...
      @Override
      public void handleEvent(Event event) {
        EmptyBaseView.this.deleteItems();
      }
    });
    ...
    deleteAllButton.addListener(SWT.Selection, new Listener() {
      ...
      @Override
      public void handleEvent(Event event) {
        EmptyBaseView.this.deleteAllItems();
      }
    });
...
//        this.getDescriptionWidget().setText(""); //$NON-NLS-1$
  ...
  public void partActivated(IWorkbenchPart arg0) {
    if (arg0 == this) {
      EmptyBaseView.this.itemsLabel.setFont(new Font(Display.getCurrent(),
          PreferenceConverter.getFontData(IEditorBase.STORE, IPreferenceConstants.DEFAULT_FONT)));
      EmptyBaseView.this.filterLabel.setFont(new Font(Display.getCurrent(),
          PreferenceConverter.getFontData(IEditorBase.STORE, IPreferenceConstants.DEFAULT_FONT)));
...
//          PreferenceConverter.getFontData(IEditorBase.STORE, IPreferenceConstants.DEFAULT_FONT)));
  ...
  protected class AbstractEditingSupport extends EditingSupport {
    ...
    protected boolean canEdit(Object element) {
      return EmptyBaseView.this.canEdit((ATableModel) element, AccessRights.WRITE);
    }
  ...
  private void addSeverityColumn() {
    if (getDataInterface() instanceof ISeverityDataModel
      ...
      severityColumn.setEditingSupport(new EditingSupport(getTableViewer()) {
        ...
        @Override
        protected boolean canEdit(Object element) {
          if (EmptyBaseView.this.canEdit(selectedEntry, AccessRights.WRITE)) {
