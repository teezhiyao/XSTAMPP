 ...
 * 
 */
public abstract class CausalFactorBaseView<T extends IDataModel> extends StandartEditorPart {
...
//  }
   ...
   */
  public CausalFactorBaseView(boolean restrict) {
  ...
  public CausalFactorBaseView(EnumSet<TableStyle> style) {
    this();
    if (style != null) {
      this.style = style;
    }
  ...
  public CausalFactorBaseView(EnumSet<TableStyle> style, String tableHeader) {
    this(style);
    this.tableHeader = tableHeader;
  }

  public CausalFactorBaseView(String tableHeader) {
    this();
    this.tableHeader = tableHeader;
  }
  ...
  private void createTableViewer(Composite tableComposite) {
    ...
    this.getTableViewer().getTable().addKeyListener(new KeyListener() {
      ...
      public void keyPressed(KeyEvent e) {
        if (((e.stateMask == SWT.CTRL) || (e.stateMask == SWT.COMMAND)) && (e.keyCode == 'a')
            && (CausalFactorBaseView.this.getTableViewer().getTable().getItemCount() > 0)) {
          CausalFactorBaseView.this.tableViewer.setSelection(
              new StructuredSelection(CausalFactorBaseView.this.getTableViewer().getElementAt(0)), true);
          CausalFactorBaseView.this.tableViewer.getTable().selectAll();
        }
      }
...
//    getTableViewer().getControl().setMenu(menu);
  ...
  private void createButtonBar() {
    ...
    deleteItemsButton.addListener(SWT.Selection, new Listener() {
      ...
      @Override
      public void handleEvent(Event event) {
        CausalFactorBaseView.this.deleteItems();
      }
    });
    ...
    deleteAllButton.addListener(SWT.Selection, new Listener() {
      ...
      @Override
      public void handleEvent(Event event) {
        CausalFactorBaseView.this.deleteAllItems();
      }
    });
  ...
  @Override
  public String getId() {
    return CausalFactorBaseView.id;
  }
...
//        this.getDescriptionWidget().setText(""); //$NON-NLS-1$
  ...
  public void partActivated(IWorkbenchPart arg0) {
    if (arg0 == this) {
      CausalFactorBaseView.this.itemsLabel.setFont(new Font(Display.getCurrent(),
          PreferenceConverter.getFontData(IEditorBase.STORE, IPreferenceConstants.DEFAULT_FONT)));
      CausalFactorBaseView.this.filterLabel.setFont(new Font(Display.getCurrent(),
          PreferenceConverter.getFontData(IEditorBase.STORE, IPreferenceConstants.DEFAULT_FONT)));
...
//          PreferenceConverter.getFontData(IEditorBase.STORE, IPreferenceConstants.DEFAULT_FONT)));
  ...
  protected class AbstractEditingSupport extends EditingSupport {
    ...
    protected boolean canEdit(Object element) {
      return CausalFactorBaseView.this.canEdit((ATableModel) element, AccessRights.WRITE);
    }
  ...
  private void addSeverityColumn() {
    if (getDataInterface() instanceof ISeverityDataModel
      ...
      severityColumn.setEditingSupport(new EditingSupport(getTableViewer()) {
        ...
        @Override
        protected boolean canEdit(Object element) {
          if (CausalFactorBaseView.this.canEdit(selectedEntry, AccessRights.WRITE)) {
