 ...
 * 
 */
public abstract class UnsafeCAView<T extends IDataModel> extends StandartEditorPart {
   ...
   */
  public UnsafeCAView(boolean restrict) {
  ...
  public UnsafeCAView(EnumSet<TableStyle> style) {
    this();
    if (style != null) {
      this.style = style;
    }
  ...
  public UnsafeCAView(EnumSet<TableStyle> style, String tableHeader) {
    this(style);
    this.tableHeader = tableHeader;
  }

  public UnsafeCAView(String tableHeader) {
    this();
    this.tableHeader = tableHeader;
  }
  ...
  private void createTableViewer(Composite tableComposite) {
    ...
    this.getTableViewer().getTable().addKeyListener(new KeyListener() {
      ...
      public void keyPressed(KeyEvent e) {
        if (((e.stateMask == SWT.CTRL) || (e.stateMask == SWT.COMMAND)) && (e.keyCode == 'a')
            && (UnsafeCAView.this.getTableViewer().getTable().getItemCount() > 0)) {
          UnsafeCAView.this.tableViewer.setSelection(
              new StructuredSelection(UnsafeCAView.this.getTableViewer().getElementAt(0)), true);
          UnsafeCAView.this.tableViewer.getTable().selectAll();
        }
      }
  ...
  private void createButtonBar() {
    ...
    deleteAllButton.addListener(SWT.Selection, new Listener() {
      ...
      @Override
      public void handleEvent(Event event) {
        UnsafeCAView.this.deleteAllItems();
      }
    });
  ...
  public void partActivated(IWorkbenchPart arg0) {
    if (arg0 == this) {
      UnsafeCAView.this.itemsLabel.setFont(new Font(Display.getCurrent(),
          PreferenceConverter.getFontData(IEditorBase.STORE, IPreferenceConstants.DEFAULT_FONT)));
  ...
  protected class AbstractEditingSupport extends EditingSupport {
    ...
    protected boolean canEdit(Object element) {
      return UnsafeCAView.this.canEdit((ATableModel) element, AccessRights.WRITE);
    }
  ...
  private void addSeverityColumn() {
    if (getDataInterface() instanceof ISeverityDataModel
      ...
      severityColumn.setEditingSupport(new EditingSupport(getTableViewer()) {
        ...
        @Override
        protected boolean canEdit(Object element) {
          if (UnsafeCAView.this.canEdit(selectedEntry, AccessRights.WRITE)) {
