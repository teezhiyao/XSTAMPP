 ...
 * 
 */
public class LTLCSVWizard extends AbstractPrivacyExportWizard {
	...
	public LTLCSVWizard() {
		...
		this.setExportPage(new CSVExportPage(filters, Messages.LTLFormulasTable + Messages.AsDataSet, Activator.PLUGIN_ID));
	}
		...
		try {
			if (this.checkError(this.checkPath(filePath))) {
				...
				XCSVExportJob export = new XCSVExportJob("Export CSV",	filePath,
						...
						model, XCSVExportJob.LTL_FORMULAS);
				export.schedule();
			} else {
				return false;
			}
