...
public class SecControlActionController implements IControlActionController {
...
//    unsafeControlAction.setPrivacyCritical(safetyConstraintDescription);
   ...
   * 
   */
  public void prepareForExport(VulLossController hazAccController,
      ControlStructureController csController,
      String defaultLabel, IExtendedDataController extendedData) {
    moveRulesInCA();
    for (ControlAction controlAction : this.controlActions) {
      ...
      for (UnsafeControlAction unsafeControlAction : controlAction
        ...
        if (linkedHazards.size() == 0) {
          ...
          for (int i = 0; i < linkedHazards.size(); i++) {
            ...
            linkString.append("V-" + linkedHazards.get(i).getNumber());
          }
        }
...
//    return action.isPrivacyCritical();
   ...
   */
  public boolean setSafetyCritical(UUID caID, boolean isSecurityCritical) {

    ControlAction action = getInternalControlAction(caID);
    if (action == null) {
      return false;
    }
    return action.setSecurityCritical(isSecurityCritical);
  }
