 ...
 */
public class SingleGridCellLinking<T extends ITableContentProvider<?>> extends AbstractGridCell {
  ...
  private class DeleteLinkButton extends CellButton {
    ...
    public void onButtonDown(Point relativeMouse, Rectangle cellBounds) {
      SingleGridCellLinking.LOGGER.debug("Delete link"); //$NON-NLS-1$
      this.provider.removeLink(SingleGridCellLinking.this.assignedId, this.tableModel.getId());
      grid.getGrid().redraw();
    }
  }
   ...
   */
  public SingleGridCellLinking(final UUID assignedId, final T publicInterface, final GridWrapper grid) {
  ...
  public void paint(final GridCellRenderer renderer, final GC gc, final NebulaGridRowWrapper item) {
    ...
    gc.setForeground(SingleGridCellLinking.TEXT_COLOR);
    ...
    if (linkedItems.isEmpty()) {
      ...
      int yOff = 0;
      int tmpLines = 1;
      boolean hovered = this.grid.getHoveredCell() == SingleGridCellLinking.this;
      ITableEntry model = linkedItems.get(this.cfRow);
      String linkText = model.getTitle();
      ...
      gc.setForeground(SingleGridCellLinking.HAZARD_LINK_COLOR);
      ...
      if (hovered && canEdit) {
        this.buttonContainer.addCellButton(new DeleteLinkButton(
            new Rectangle(xOff + 4, yOff + 2, SingleGridCellLinking.DELETE_LINK_BUTTON_SIZE,
                SingleGridCellLinking.DELETE_LINK_BUTTON_SIZE),
            GridWrapper.getDeleteButton16(), model, this.publicInterface));
      }
      xOff += SingleGridCellLinking.DELETE_LINK_BUTTON_SIZE;
  ...
  public void onMouseDown(final MouseEvent event, final Point relativeMouse,
    ...
    SingleGridCellLinking.LOGGER.debug("Link clicked"); //$NON-NLS-1$
